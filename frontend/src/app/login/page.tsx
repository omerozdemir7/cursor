    ...
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-xl border border-zinc-200 dark:border-zinc-800 p-6 bg-white dark:bg-zinc-900 shadow-sm">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <button disabled={loading} className="w-full rounded-md bg-red-600 dark:bg-orange-500 text-white py-2 text-sm font-medium hover:opacity-90 transition">{loading ? "Signing in..." : "Sign In"}</button>
        </form>
        <p className="text-sm mt-4 text-zinc-600 dark:text-zinc-400">
          No account? <Link href="/register" className="text-red-600 dark:text-orange-400 hover:underline">Register</Link>
        </p>
      </div>
    </div>
  );
